...
function mapStationWithDistance(station, latT, lonT) {
    const lat = parseFloat(String(station['Latitud'] || '').replace(',', '.'));
    const lon = parseFloat(String(station['Longitud (WGS84)'] || '').replace(',', '.'));

    if (!Number.isFinite(lat) || !Number.isFinite(lon)) return null;

    const d = getDistance(latT, lonT, lat, lon);
    return { ...station, distance: d, lat, lon };
}
...
async function getNearbyFuelStations(latT, lonT, limit = 10) {
    try {
        ...
        return fuelList
            .map((s) => mapStationWithDistance(s, latT, lonT))
            .filter(Boolean)
